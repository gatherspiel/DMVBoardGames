  ...
  attachHandlersToShadowRoot(shadowRoot) {
    ...
    shadowRoot.addEventListener("click", (event)=>{
      ...
      if(event.target.type === "checkbox"){
        self.updateData({
          description: shadowRoot.getElementById(GROUP_DESCRIPTION_INPUT)?.value,
        ...
        const groupDescription = shadowRoot.getElementById(GROUP_DESCRIPTION_INPUT)?.value;
  ...
  render(groupData) {
    ...
    ${
      groupData.weeklyEventData.length === 0
        ? ``
        : `  
          ${groupData.weeklyEventData.map((event) => {
