...
export function convertDateListToRange(dates) {
  ...
  if (dates.length > 1) {
    const endMonth =
      months[parseInt(dates[dates.length - 1].substring(5, 7)) - 1];
...
export function convertDateAndDayToDisplayString(date, dayOfWeek) {
  const dateSplit = date.split("-");
...
export function validateAddress(addressStr) {
  const split = addressStr.split(",");

  if (split.length !== 3 || split[2].trim().split(" ").length !== 2) {
    throw new Error(
      "Invalid address format. Address should be in the form 'street, city, state_code zip_code ",
    );
  }
